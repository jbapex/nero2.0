import React, { memo } from 'react';
    ...
    const ImageGeneratorNode = memo(({ data, id }) => {
        const { onUpdateNodeData, presets, inputData, selectedPresetId } = data;
        const { toast } = useToast();
        const [isLoading, setIsLoading] = React.useState(false);
        const [lastImageUrl, setLastImageUrl] = React.useState(data.lastImageUrl || null);
        ...
        const handleGenerateImage = async () => {
            ...
            try {
                ...
                if (functionData.images && functionData.images.length > 0) {
                    const imageUrl = functionData.images[0].url_publica;
                    setLastImageUrl(imageUrl);
                    onUpdateNodeData(id, {
                        lastImageUrl: imageUrl,
                        output: {
                            id: functionData.job_id,
                            data: functionData.images,
                        },
                    });
            ...
            <Card className="w-64 border-2 border-pink-500/50 shadow-lg">
                ...
                <CardContent className="p-3 space-y-3">
                    ...
                    <div className="w-full aspect-square bg-muted rounded-md flex items-center justify-center overflow-hidden">
                        {isLoading ? <Loader2 className="w-8 h-8 animate-spin text-pink-500" /> : 
                         lastImageUrl ? <img src={lastImageUrl} alt="Imagem gerada" className="w-full h-full object-cover" /> :
                         <Paintbrush className="w-8 h-8 text-muted-foreground" />}
                    </div>

                    <Button onClick={handleGenerateImage} disabled={isLoading || !campaignId || !selectedPresetId} className="w-full">
